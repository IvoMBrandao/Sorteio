...
export function SavedLists({ lists, onDelete, onLoad, onUpdate }: SavedListsProps) {
  ...
  const handleDelete = (list: SavedList) => {
    Alert.alert(
      t('names.savedLists.deleteAlertTitle'),
      t('names.savedLists.deleteAlertMsg', { title: list.title }),
      [
  ...
  const handleLoad = (list: SavedList) => {
    Alert.alert(
      t('names.savedLists.loadAlertTitle'),
      t('names.savedLists.loadAlertMsg', { title: list.title }),
      [
        { text: t('common.cancel'), style: 'cancel' },
        { 
          text: t('names.savedLists.addAction'), 
          onPress: () => onLoad(list) 
        }
      ]
    );
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>
        {t('names.savedLists.sectionTitle', { count: lists.length })}
      </Text>
          ...
          <View key={list.id} style={styles.card}>
            ...
            <View>
              ...
              <Text style={styles.cardCount}>
                {t('names.savedLists.namesCount', { count: list.names.length })}
              </Text>
            </View>
