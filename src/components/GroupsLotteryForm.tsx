...
export function GroupsLotteryForm({ onClose }: GroupsLotteryFormProps) {
  ...
  return (
    <View style={styles.container}>
      ...
      <FormCard>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.sourcesContainer}>
          <TouchableOpacity 
            ...
            onPress={() => setSelectedSourceId('loose')}
          >
            ...
            <View>
              <Text style={[styles.sourceTitle, selectedSourceId === 'loose' && styles.selectedSourceText]}>
                {t('sortear.groups.looseNames')}
              </Text>
              <Text style={[styles.sourceCount, selectedSourceId === 'loose' && styles.selectedSourceText]}>
                {t('sortear.groups.peopleCount', { count: looseNames.length })}
              </Text>
            </View>
          </TouchableOpacity>
          ...
          {savedLists.map(list => (
            <TouchableOpacity 
              ...
              onPress={() => setSelectedSourceId(list.id)}
            >
              <View style={styles.sourceHeader}>
              ...
              <View>
                ...
                <Text style={[styles.sourceCount, selectedSourceId === list.id && styles.selectedSourceText]}>
                  {t('sortear.groups.peopleCount', { count: list.names.length })}
                </Text>
              </View>
            </TouchableOpacity>
          ))}
      ...
      <FormCard>
        <Text style={styles.sectionTitle}>
          {t('sortear.groups.configTitle')}
        </Text>
        <NumberInput 
          label={t('sortear.groups.membersLabel')} 
          value={membersPerGroup} 
          onChangeValue={setMembersPerGroup} 
          min={1} 
          max={Math.max(activeNames.length, 1)} 
        />
          ...
          <View style={styles.infoBox}>
            <View style={styles.infoRow}>
              ...
              <Text style={styles.infoText}>
                {t('sortear.groups.infoTotal', { 
                  total: activeNames.length, 
                  groups: calculationInfo.totalGroups 
                })}
              </Text>
            </View>
            {calculationInfo.hasImperfectGroup ? (
              <View style={[styles.infoRow, styles.warningRow]}>
                ...
                <Text style={styles.warningText}>
                  {t('sortear.groups.infoRemainder', { count: calculationInfo.remainder })}
                </Text>
              </View>
            ) : (
              <Text style={styles.successText}>
                {t('sortear.groups.infoPerfect')}
              </Text>
            )}
          </View>
        )}
        ...
        <View style={styles.row}>
          <View style={{ flex: 1 }}>
            <Text style={styles.label}>
              {t('sortear.groups.distributeEvenly')}
            </Text>
            <Text style={styles.subLabel}>
              {t('sortear.groups.distributeSub')}
            </Text>
          </View>
          <Switch 
            value={distributeEvenly} 
            onValueChange={setDistributeEvenly} 
            trackColor={{ false: '#e2e8f0', true: '#6366f1' }} 
            thumbColor={distributeEvenly ? '#ffffff' : '#94a3b8'} 
          />
        </View>
      </FormCard>
